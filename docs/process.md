...
To make sure the box-shadow of the button stayed in it's initial location, I created another child element on top of the knob and gave it a pseudo-element for the little rotation indicator. Now, the rotate animation only happens on this new child element, and the rotation indicator moves along with it to the 'correct' location. There was one other issue that annoyed me quite a bit. I created an 'undo' animation, which basically just reverses the animation when the form is not valid anymore, but this means that on window load, the form is also not valid! This results in the user seeing the buttons for a split second before they disappear. 

This issue would be easy to solve with some Javascript (again), but we're here to learn about CSS! I looked into solutions and found [this](https://codepen.io/narvin/pen/abBYOLP) codepen in which someone faced the same type of issue I was dealing with. I tried it out myself but found it to be very hacky and confusing. Why isn't there an easier way to check this! Spoiler alert, there is: but only for Firefox. Firefox created a (still experimental) selector called `:user-valid`. This selector is kind of like the `:valid` selector, except it makes sure it was all done by the hand of an actual user! (kind of similar to `:focus-visible` and `:focus-within`). I once again tried to get this to work by enabling the needed settings in my Firefox configuration, but was unable to get it to work. So, I came up with something else: a simple loader that disappears after the 'reverse' animation of the buttons are done!

Now that I'm looking back at the things I was focused on, I think I migth have been able to get the slider to work. I think I had somewhat of a tunnelvision when it came to creating the knob button. Using the slider came with an additional JS script, which added a data attritbute to the HTML tag! I wasn't familiar with the attribute selector before, but I could have used the selector to change the rotation of the button instead of a checkbox.
